...
import 'package:gg/gen_pdf.dart';
  ...
  try {
    ...
    if (pageImage != null) {
      ...
      try {
        ...
        final ByteData data = await rootBundle.load('assets/img/logo-4.png');
        final Uint8List bytes = data.buffer.asUint8List();
        final img.Image? assetImage = img.decodeImage(bytes);

        if (assetImage != null) {
          // Print the asset image
          byte.addAll(generator.image(assetImage, align: PosAlign.left));
        }

        // Print the PDF page image
        byte.addAll(generator.image(pageImage, align: PosAlign.left));
        byte.addAll(generator.cut());
        byte.addAll(generator.drawer());
      } catch (e) {
        print(e);
      }
    }
